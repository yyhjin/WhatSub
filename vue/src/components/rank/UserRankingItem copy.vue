  ...
  <div class="main_card">
    <v-card class="home_card_b" elevation="3" outlined>
      ...
      <div class="rollingbanner">
        ...
        <div class="wrap">
          <h5 class="current">
            {{ sand.data[0].ranking }}위 {{ sand.data[0].userName }} {{ sand.data[0].cnt }}개
            {{ sand.data[1].ranking }}위 {{ sand.data[1].userName }} {{ sand.data[1].cnt }}개
            {{ sand.data[2].ranking }}위 {{ sand.data[2].userName }} {{ sand.data[2].cnt }}개
          </h5>
          <h5 class="next">
            {{ sand.data[1].ranking }}위 {{ sand.data[1].userName }} {{ sand.data[1].cnt }}개
          </h5>
...
export default {
  ...
  methods: {
    rollingCallback() {
      //.prev 클래스 삭제
      document
        .querySelectorAll(".rollingbanner .prev")
        .forEach((selected) => selected.classList.remove("prev"));
      ...
      next.forEach((selected) => {
        if (selected.nextElementSibling == null) {
          document
            .querySelectorAll(".rollingbanner .wrap h5:first-child")
            .forEach((sel) => sel.classList.add("next"));
        } else {
          //목록 처음 요소를 다음 요소로 선택
